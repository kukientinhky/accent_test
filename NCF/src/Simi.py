...
class Simi(AccentTemplate):
    ...
    def find_counterfactual_multiple_k(user, ks, model, data, args):
        ...
        """
            we have scores for all items not visited by the user
            we find influences of each item in top k on the test case
        """
        for i in range(ks[-1]):
            test_idx = user * ks[-1] + i # get the index of the test case
