...
class GenericNeuralNet(object):
    ...
    def reset_datasets(self):
        ...
        self.data_sets.validation.reset_batch()
    ...
    def print_model_eval(self):
        ...
        print('Loss on test idx %d: %s' % (test_idx, test_loss_val_test))
    ...
    def train(self, num_steps, 
        ...
        if load_checkpoints:
            self.load_checkpoint(load_checkpoints, do_checks=False)
        else:
            load_checkpoints=0
        ...
        for step in xrange(load_checkpoints+1, num_steps):
            ...
            if step>100000/5:
                # if (step + 1) % (10000/5) == 0 or (step + 1) == num_steps:
                if (step + 1) == num_steps:
    ...
    def load_checkpoint_params(self, iter_to_load, do_checks=True):
        checkpoint_to_load = "%s-%s" % (self.checkpoint_file, iter_to_load)
        params_saver = tf.train.Saver(self.params)
        params_saver.restore(self.sess, checkpoint_to_load)
    ...
    def get_train_sgd_op(self, total_loss, global_step, learning_rate=0.0001):
        """
        Return train_sgd_op
        """
    ...
    def minibatch_hessian_vector_val(self, v):
        ...
        # while True:
        #     try:
        #         decomp = np.linalg.cholesky(hessian_vector_val)
        #         print("hessian is positive definite now")
        #         break
        #     except:
        #         print("hessian is not positive definite yet")
        #         hessian_vector_val = [a + self.damping * b for (a,b) in zip(hessian_vector_val, v)]
        # try:
        #     decomp = np.linalg.cholesky(hessian_vector_val)
        #     print("hessian is positive definite now")
        # except:
        #     print("hessian is not positive definite now")
    ...
    def update_train_x(self, new_train_x):
        ...
        self.data_sets = Datasets(train=new_train, validation=self.data_sets.validation, test=self.data_sets.test)
        self.all_train_feed_dict = self.fill_feed_dict_with_all_ex(self.data_sets.train)                
        self.reset_datasets()
    ...
    def update_test_x_y(self, new_test_x, new_test_y):
        ...
        self.data_sets = Datasets(train=self.data_sets.train, validation=self.data_sets.validation, test=new_test)
        self.all_test_feed_dict = self.fill_feed_dict_with_all_ex(self.data_sets.test)                
